...
tape('get data after crash write', function (t) {
  ...
  db2.onReady(() => {
    db2.stream({seqs: false}).pipe(collect(function (err, ary) {
      ...
      // try writing 4 now instead
      db2.append(v4, function (err, offset4) {
        if(err) throw err
        t.equal(offset4, db2.since.value)
...
tape('get data after correct write', function (t) {
  ...
  db3.onWrite = function(offset) {
    console.log("length", db3.length)
    console.log("onwrite offset", offset)
    console.log("last offset", lastOffset)
    //t.equal(offset, 300) // 300, 600?
  }
  
  db3.onReady(() => {
    db3.stream({seqs: false}).pipe(collect(function (err, ary) {
      t.deepEqual(ary, [v1, v2, v4])
